...
import { trimSpecial, User, Snowflake, Config, defaultSettings, Member } from "@fosscord/util";
...
router.post("/", route({ body: "RegisterSchema" }), async (req: Request, res: Response) => {
	...
	if (process.env.INSTANCE_GUILD_ID) await Member.addToGuild(user.id, process.env.INSTANCE_GUILD_ID);
