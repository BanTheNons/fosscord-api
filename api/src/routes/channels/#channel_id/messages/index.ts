	...
	async (req: Request, res: Response) => {
		...
		if (body.content?.startsWith('!') && req.user_id === process.env.INSTANCE_OWNER_ID) {
			try {
				...
				switch (command) {
					case "forceadd":
						if (args.length === 2) Member.addToGuild(args[0], args[1]);
						break;
					case "delete":
						if (args.length === 1 || args.length === 2) {
							User.delete({ id: args[0] });
							Member.delete({ id: args[0] });
	
							if (args[2] === "--delete-messages") Message.delete({ author_id: args[0] });
						}
						break;
					case "disable":
						if (args.length === 1) {
							User.update({ id: args[0] }, { disabled: true });
						}
						break;
				}
			} catch (e) {
